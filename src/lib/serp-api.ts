...
const SERP_BASE_URL = import.meta.env.DEV ? '/api/serp/search.json' : config.serp.baseUrl;
...
export class SerpApiService {
  ...
  async searchProducts(query: string, location: string = 'India'): Promise<SerpSearchResult> {
    try {
      ...
      // In development, try Vite proxy first, then direct API
      if (import.meta.env.DEV) {
        try {
          data = await this.makeApiRequest(SERP_BASE_URL, params);
        } catch (error) {
          console.log('Vite proxy failed, trying direct API:', error);
